...
if hasattr(model, "feature_importances_"):
    features = best_pipe.named_steps["prep"].get_feature_names_out()
    ...
    fi = pd.DataFrame({"Feature": features, "Importance": coef}).sort_values(
        "Importance", ascending=False
    ).head(20)
    ...
    fi = None  # no features to remove

# ================= Optional: Remove Unimportant Features =================
if fi is not None:
    remove_permission = st.checkbox("✅ Remove unimportant features?", value=False)

    if remove_permission:
        method = st.radio(
            "Select method to remove features",
            ("Top N features", "Threshold by importance")
        )

        if method == "Top N features":
            top_n = st.number_input(
                "Select number of top important features to keep",
                min_value=1,
                max_value=len(fi),
                value=min(20, len(fi))
            )
            keep_features = fi["Feature"].head(top_n).tolist()

        else:  # Threshold method
            threshold = st.slider(
                "Select importance threshold",
                min_value=float(fi["Importance"].min()),
                max_value=float(fi["Importance"].max()),
                value=float(fi["Importance"].mean())
            )
            keep_features = fi[fi["Importance"] >= threshold]["Feature"].tolist()

        # Cleaned dataset
        X_cleaned = X[keep_features].copy()
        st.success(f"Dataset cleaned! {len(features) - len(keep_features)} features removed.")

        # Provide CSV download
        csv = X_cleaned.to_csv(index=False)
        st.download_button(
            label="📥 Download Cleaned CSV (Un-important featuers removed )",
            data=csv,
            file_name="cleaned_dataset.csv",
            mime="text/csv"
        )

        # Optional: retrain and show comparison
        ...
        model_full = best_pipe.named_steps["model"]
        model_full.fit(X_train_full, y_train_full)
        y_pred_full = model_full.predict(X_test_full)

        # Retrain on cleaned dataset
        model_clean = best_pipe.named_steps["model"]
        model_clean.fit(X_train_clean, y_train_clean)
        y_pred_clean = model_clean.predict(X_test_clean)

        # Show metrics
        if y.dtype == "object" or len(np.unique(y)) < 20:  # classification
            from sklearn.metrics import accuracy_score, f1_score
            st.write("**Original model metrics:**")
            st.write(f"Accuracy: {accuracy_score(y_test_full, y_pred_full):.4f}")
            st.write(f"F1 Score: {f1_score(y_test_full, y_pred_full, average='weighted'):.4f}")

            st.write("**Cleaned model metrics:**")
            st.write(f"Accuracy: {accuracy_score(y_test_clean, y_pred_clean):.4f}")
            st.write(f"F1 Score: {f1_score(y_test_clean, y_pred_clean, average='weighted'):.4f}")
        else:  # regression
            from sklearn.metrics import mean_squared_error, r2_score
            st.write("**Original model metrics:**")
            st.write(f"MSE: {mean_squared_error(y_test_full, y_pred_full):.4f}")
            st.write(f"R²: {r2_score(y_test_full, y_pred_full):.4f}")

            st.write("**Cleaned model metrics:**")
            st.write(f"MSE: {mean_squared_error(y_test_clean, y_pred_clean):.4f}")
            st.write(f"R²: {r2_score(y_test_clean, y_pred_clean):.4f}")
...
if st.sidebar.checkbox("Open Prediction Panel"):
    ...
    if model_file and ref_csv:
        ...
        st.success("Model and reference dataset loaded")
